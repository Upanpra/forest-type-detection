...
from src.MultipleImageDataloader import multi_image_data_loader, multi_image_data_loader_balanced
...
from src.data_loader import transforms_aug, data_loader, data_loader_balanced
import time
...
def main():
    ...
    # Define model name
    model_name = "DroughtCNN" #"conv_next_model" #resnet18
    ...
    if len(train_folder) == 1:
        assert len(test_folder) == 1
        train_loader, test_loader = data_loader_balanced(train_folder[0], test_folder[0], INPUT_SIZE, BATCH_SIZE, mean, std)
    else:
        train_loader, test_loader = multi_image_data_loader_balanced(train_folder, test_folder, INPUT_SIZE, BATCH_SIZE, mean, std)
